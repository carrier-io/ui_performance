...
from tools import api_tools, auth, LokiLogFetcher, MinioClient
...
def write_test_run_logs_to_minio_bucket(report: UIReport) -> str:
    report_id = report.uid
    project_id = report.project_id
    test_name = report.name
    logs_query = '{report_id="%s",project="%s"}' % (report_id, project_id)
    ...
    try:
        ...
        minio_client = MinioClient.from_project_id(project_id, **s3_settings)
